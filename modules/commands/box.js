...
module.exports.handleEvent = async ({ api, event, args }) => {
    ...
    try {
        ...
        // Kiểm tra xem nội dung file có rỗng hay không
        if (fileContent.trim() === "") {
            totalChat = {};
        } else {
            totalChat = JSON.parse(fileContent);
        }
    } catch (error) {
        console.error("Lỗi khi đọc file totalChat.json:", error);
        // Tạo lại file nếu có lỗi
        fs.writeFileSync(totalPath, JSON.stringify({}));
        totalChat = {};
    }

    // Kiểm tra sự kiện và cập nhật dữ liệu
    if (!totalChat[event.threadID]) {
        totalChat[event.threadID] = { time: Date.now(), count: 0, ytd: 0 };
    }

    if (Date.now() - totalChat[event.threadID].time > (_24hours * 2)) {
        let sl = (await api.getThreadInfo(event.threadID)).messageCount;
        totalChat[event.threadID].time = Date.now() - _24hours;
        totalChat[event.threadID].count = sl;
        totalChat[event.threadID].ytd = sl - totalChat[event.threadID].count;

        // Ghi lại dữ liệu vào file
        try {
            fs.writeFileSync(totalPath, JSON.stringify(totalChat, 2));
        } catch (error) {
            console.error("Lỗi khi ghi file totalChat.json:", error);
        }
    }
};
module.exports.run = async ({
    ...
    if (args[0] == "image") {
        ...
        var callback = () => api.changeGroupImage(fs.createReadStream(__dirname + "/cache/1.png"), event.threadID, () => fs.unlinkSync(__dirname + "/cache/8.png"));
        return request(encodeURI(event.messageReply.attachments[0].url)).pipe(fs.createWriteStream(__dirname + '/cache/8.png')).on('close', () => callback());
    };
    ...
    if (args[0] == "info") {
        ...
        return request(encodeURI(`${threadInfo.imageSrc}`))
            .pipe(fs.createWriteStream(__dirname + '/cache/8.png'))
            .on('close', () => callback());
    }
