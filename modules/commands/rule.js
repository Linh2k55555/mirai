...
module.exports.run = ({ event, api, args, permission }) => {
    ...
    switch (args[0]) {
        case "add": {
            if (permission === 0) return api.sendMessage("[Rule] Bạn không đủ quyền hạn để có thể sử dụng thêm luật!", threadID, messageID);
        ...
        case "delete": {
            if (!isNaN(content) && content > 0) {
                if (permission === 0) return api.sendMessage("[Rule] Bạn không đủ quyền hạn để có thể sử dụng xóa luật!", threadID, messageID);
